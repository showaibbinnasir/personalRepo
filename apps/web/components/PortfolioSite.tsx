...
export default function PortfolioSite({ data }: { data: Portfolio }) {
  ...
  return <main style={styles}>
    ...
    <section id="work" className="section projects-section">
      ...
      {projects.length ? <div className="project-grid">{projects.map((p,i)=><article className={`project-card ${p.featured?'featured':''}`} key={p.id}>{p.imageUrl ? <img src={p.imageUrl} alt=""/> : <div className="project-image-placeholder">PROJECT IMAGE</div>}<div className="project-copy"><small>0{i+1} / {p.category || 'PROJECT'}</small><h3>{p.title}</h3><p>{p.summary}</p><div className="tags">{p.tech.map(t=><span key={t}>{t}</span>)}</div><div className="project-links">{p.liveUrl && <a href={p.liveUrl} target="_blank">Live ↗</a>}{p.githubUrl && <a href={p.githubUrl} target="_blank">Code ↗</a>}</div></div></article>)}</div> : <div className="empty-public">PROJECTS WILL APPEAR HERE WHEN ADDED FROM THE DASHBOARD.</div>}
    </section>
